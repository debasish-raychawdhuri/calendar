...
// Outlook Calendar integration methods for CalendarUI
pub async fn handle_outlook_calendar(
    outlook_client: &mut Option<OutlookCalendarClient>,
    ...
    if outlook_client.is_none() {
        println!("Outlook Calendar is not configured.");
        ...
        if input.trim().to_lowercase() == "y" {
            setup_outlook_calendar(outlook_client).await?;
        }
    } else if !outlook_client.as_ref().unwrap().is_authenticated() {
        println!("Outlook Calendar is configured but not authenticated.");
        ...
        if input.trim().to_lowercase() == "y" {
            authenticate_outlook_calendar(outlook_client).await?;
        }
    } else {
        println!("Outlook Calendar is configured and authenticated.");
        ...
        match input.trim() {
            "1" => {
                ...
                let count = outlook_client.as_mut().unwrap()
            ...
            "3" => {
                ...
                let token_path = std::path::PathBuf::from(dirs::home_dir().unwrap_or_default())
                    .join(".calendar_outlook_token.json");
                ...
                if let Some(creds) = OutlookCredentials::load() {
                    *outlook_client = Some(OutlookCalendarClient::new(&creds.client_id, &creds.client_secret));
                }
...
async fn setup_outlook_calendar(outlook_client: &mut Option<OutlookCalendarClient>) -> Result<(), DbError> {
    println!("\nTo set up Outlook Calendar integration, you need to create OAuth credentials in Azure Portal.");
    println!("Follow these steps:");
    println!("1. Go to https://portal.azure.com/");
    println!("2. Navigate to Azure Active Directory > App registrations");
    println!("3. Click 'New registration'");
    println!("4. Enter a name for your app");
    println!("5. Select 'Accounts in any organizational directory and personal Microsoft accounts'");
    println!("6. Add http://localhost:8080 as a redirect URI (Web platform)");
    println!("7. Go to 'API permissions' and add Microsoft Graph > Calendars.Read");
    println!("8. Go to 'Certificates & secrets' and create a new client secret");
    println!("9. Copy the Application (client) ID and Client secret value");
    ...
    *outlook_client = Some(OutlookCalendarClient::new(
    ...
    if input.trim().to_lowercase() == "y" {
        ...
        println!("You can authenticate later by pressing 'O' in the calendar view.");
        println!("Press Enter to continue...");
        let mut input = String::new();
        std::io::stdin().read_line(&mut input).unwrap_or_default();
    }
...
async fn authenticate_outlook_calendar(outlook_client: &mut Option<OutlookCalendarClient>) -> Result<(), DbError> {
    if outlook_client.is_none() {
        println!("Outlook Calendar is not configured.");
        println!("Press Enter to continue...");
        let mut input = String::new();
    ...
    let outlook_client_ref = outlook_client.as_ref().unwrap();
    
    // Start the OAuth flow
    let (auth_url, _csrf_token, pkce_challenge) = outlook_client_ref.start_auth_flow();
    ...
    if let Some(code) = auth_code {
        ...
        let outlook_client_mut = outlook_client.as_mut().unwrap();
        
        match outlook_client_mut.complete_auth_flow(&code, pkce_challenge).await {
            Ok(_) => {
