...
pub struct CalendarUI {
    ...
    outlook_client: Option<OutlookCalendarClient>,
}
impl CalendarUI {
    pub fn new(db: Arc<Mutex<Database>>) -> Self {
        ...
        // Try to load Outlook credentials and create client
        let outlook_client = OutlookCredentials::load().map(|creds| {
            OutlookCalendarClient::new(&creds.client_id, &creds.client_secret)
        });
    ...
    fn draw_calendar(&self) {
        ...
        mvprintw(
            LINES() - 2,
            2,
            "Arrow keys: Navigate | Enter: Add | Tab: Events | G: Google | O: Outlook | q: Quit",
        );
        attroff(A_BOLD());
    ...
    async fn handle_calendar_input(&mut self, ch: i32) -> Result<(), DbError> {
        match ch {
            ...
            111 | 79 => { // 'o' or 'O' for Outlook Calendar
                self.handle_outlook_calendar().await?;
            }
            _ => {}
        }
    ...
    async fn handle_outlook_calendar(&mut self) -> Result<(), DbError> {
        ...
        // Call the Outlook Calendar handler with the cloned data
        let result = crate::ui_outlook::handle_outlook_calendar(
            outlook_client,
            &db,
            year,
            month,
        ...
        // Reload events after Outlook Calendar operations
        if result.is_ok() {
            self.load_events().await?;
        }
